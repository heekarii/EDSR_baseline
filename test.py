...
# 크기 일치시키기 (input_image와 output_image의 해상도를 맞춤)
def match_size(tensor, reference_tensor):
    _, _, h_ref, w_ref = reference_tensor.shape
    image_np = tensor.squeeze(0).cpu().detach().numpy().transpose(1, 2, 0)  # (C, H, W) -> (H, W, C)
    image_np = cv2.resize(image_np, (w_ref, h_ref), interpolation=cv2.INTER_CUBIC)
    image_tensor = transforms.ToTensor()(image_np).unsqueeze(0)  # 다시 텐서로 변환
    return image_tensor.to(reference_tensor.device)
...
if __name__ == "__main__":
    ...
    # PSNR 및 SSIM 계산
    psnr_value = calculate_psnr(input_image, output_image)
    ssim_value = ssim(output_image, input_image, data_range=1.0, size_average=True).item()
